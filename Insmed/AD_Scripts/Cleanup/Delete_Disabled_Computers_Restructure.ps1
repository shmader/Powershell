...
#
# Purpose: 
# - Finds all computers in the new "Pending Delete" OU and checks that they are BOTH disabled AND logon is over 90 days ago.
...
$logfilepath= "C:\temp\AD_Reports\Computers_Cleanup\AD_Computers_Deletion_Log_Restructure"+$todaysdate+".txt"
$logFile = $logFilePath
...
$computers = Get-ADComputer -Filter * -SearchBase "OU=Computers Pending Delete,OU=Disabled Computers,OU=Insmed Computers,DC=insmed,DC=local" -SearchScope Subtree -Properties *
