...
$logfilepath= "C:\temp\AD_Disabled_Computers_Move_To_Delete_OU_Log_"+$todaysdate+".txt"
$logFile = $logFilePath
...
$computers = Get-ADComputer -Filter * -SearchBase "OU=Disabled_Computers,DC=sjno,DC=net" -SearchScope Subtree -Properties *
...
foreach ($computer in $computers) {
    ...
    if ($computer.LastLogonDate -lt $90days) {
        ...
        if ($computer.Enabled -eq $false) {
            ...
            #Move-ADObject -TargetPath "OU=Pending_Delete,DC=sjno,DC=net" -Identity $computer.DistinguishedName
